...
glConsSpec = describe "glCons" $ do
  it "adding element to empty list" $
    glCons bparker empty `shouldBe` singleton bparker
  it "adding element to non-empty list" $ do
    glCons bbow (GL [bparker] (empFun bparker)) `shouldBe` GL [bbow,bparker] (empFun bparker + empFun bbow)
    glCons btefay brads `shouldBe` GL (btefay:glGuests brads) (empFun btefay + glFun brads)
    glCons mfurness founders `shouldBe` GL (mfurness:glGuests founders) (empFun mfurness + glFun founders)
...
nextLevelSpec =
  describe "nextLevel" $ do
    it "employee has no subtrees" $
      nextLevel mfurness [] `shouldBe` (singleton mfurness, empty)
    it "employee has single subtree" $ do
      nextLevel fferraioli [(singleton bbow, singleton jottaway)] -- 77 [(1, 65)]
        `shouldBe`
          (glCons fferraioli (singleton jottaway), singleton bbow) -- I don't understand why snd isn't singlton jottaway...
      nextLevel jottaway [(engineers, brads)]
        `shouldBe`
          (glCons jottaway brads, engineers)
    it "employee has many subtrees" $
      nextLevel bbow [
          ...
          `shouldBe`
            (
              glCons bbow (singleton jottaway <> singleton btefay <> singleton fferraioli),
...
maxFunSpec :: Spec
maxFunSpec =
  describe "maxFunSpec" $ do
    it "single node tree" $
      maxFun (Node dsowter []) `shouldBe` singleton dsowter
...
companyFileSpec =
  describe "evaluating company.txt" $
    it "should have 458 guests, with a fun total of 21669" $
      unsafePerformIO runIO `shouldBe` (21669, 458)

...
xorSpec = describe "xor" $ do
  it "xor [] == False" $ xor [] `shouldBe` False
  it "xor [False] == False" $ xor [False] `shouldBe` False
...
map'Spec :: Spec
map'Spec = describe "map'" $ do
  it "map' f [] == map f []" $ map' (+ 1) [] `shouldBe` map (+ 1) []
  it "map' f [x] == map f [x]" $ map' (* 2) [2] `shouldBe` map (* 2) [2]
...
myFoldlSpec = describe "myFoldlSpec" $ do
  ...
  it "myFoldl f b [x,y,z] == foldl f b [x,y,z]" $ do
    ...
    myFoldl (&&) True [True, True, True]
      `shouldBe` foldl (&&) True [True, True, True]

sieveSundaramSpec :: Spec
sieveSundaramSpec = describe "sieveSundaram" $ do
  it "sieveSundaram 1 == [3]" $ sieveSundaram 1 `shouldBe` [3]

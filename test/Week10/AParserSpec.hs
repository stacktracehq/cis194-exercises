...
applicativeSpec = describe "Applicative" $ do
  ...
  it "is homomorphic" $ require $ property $ do
    c <- forAll Gen.unicode
    s <- forAll unicodeString
...
intPairSpec =
  describe "intPair"
    $ it
        "reads two integer values separated by a space and returns the integer values in a list"
    $ require
    $ property
    $ do

...
parseMessageSpec =
  describe "parseMessage" $ it "converts a log line into a LogMessage" $ do
    parseMessage "E 2 562 help help"
      `shouldBe` LogMessage (Error 2) 562 "help help"
    parseMessage "I 29 la la la" `shouldBe` LogMessage Info 29 "la la la"
    parseMessage "W 45 A warning" `shouldBe` LogMessage Warning 45 "A warning"
...
parseSpec = describe "parse" $ do
  ...
  it "returns empty list for empty string" $ parse "" `shouldBe` []
  it "returns singleton list for input without new lines"
    $          parse "E 2 562 help help I 29 la la la"
    `shouldBe` [LogMessage (Error 2) 562 "help help I 29 la la la"]
...
insertSpec = describe "insert" $ do
  ...
  it "inserts a LogMessage into an empty tree, producing a single-node tree"
    $          insert messageA Leaf
    `shouldBe` Node Leaf messageA Leaf
  it
      "inserts a new LogMessage into a sorted MessageTree, producing a new sorted MessageTree"
    $ do
        insert messageC tree
          `shouldBe` Node
                       (Node Leaf messageB (Node Leaf messageC Leaf))
                       messageD
                       Leaf
        insert messageA tree
          `shouldBe` Node
                       (Node (Node Leaf messageA Leaf) messageB Leaf)
                       messageD
                       Leaf
...
buildSpec = describe "build" $ do
  ...
  it "builds a complete MessageTree from a list of unqique messages" $ do
    let inputOne = [messageA, messageG, messageB]
    let expectedOne =
          Node Leaf messageA (Node (Node Leaf messageB Leaf) messageG Leaf)
    build inputOne `shouldBe` expectedOne

    let inputTwo = [messageC, messageA, messageD]
    let expectedTwo =
          Node (Node Leaf messageA Leaf) messageC (Node Leaf messageD Leaf)
    build inputTwo `shouldBe` expectedTwo

inOrderSpec :: Spec
inOrderSpec = describe "inOrder" $ do
  it "returns an empty list when given an empty MessageTree"
    $          inOrder Leaf
    `shouldBe` []
  it "returns an singlton list when given a single-node tree"
    $          inOrder (Node Leaf messageA Leaf)
    `shouldBe` [messageA]
  it
    ...
    $ do
        let input = Node
              (Node (Node Leaf messageA Leaf) messageB (Node Leaf messageC Leaf)
              )
              messageD
              (Node (Node Leaf messageE Leaf) messageF (Node Leaf messageG Leaf)
              )
        let expected =
              [ messageA
              , messageB
...
whatWentWrongSpec =
  describe "whatWentWrong"
    $ it
        "takes an unsorted list of LogMessage s, and returns a list of the messages corresponding to any errors with a severity of 50 or greater, sorted by timestamp"
    $ do
        let input =
              [ messageF
              , messageA

...
import           Week11.AParser                 ( runParser
                                                , satisfy
                                                )
import           Week11.SExpr                   ( Atom(..)
                                                , SExpr(..)
                                                , ident
                                                , oneOrMore
                                                , parseSExpr
...
oneOrMoreSpec :: Spec
oneOrMoreSpec = describe "oneOrMore" $ do
  it "parses one or more occurences from start" $ require $ property $ do
    uppers <- forAll $ upperCaseString (1, 100)
    lowers <- forAll $ lowerCaseString (0, 100)
    let input = uppers <> lowers
    runParser (oneOrMore (satisfy isUpper)) input === Just (uppers, lowers)
  it "fails when no matches at start" $ require $ property $ do
...
identSpec = describe "ident" $ do
  ...
  it "fails when first char is not an alpha" $ require $ property $ do
...
atom :: String -> Atom
atom input = case trim input of
  str@(s : _) -> if isAlpha s then I str else N (read str)
...
parseSExprSpec :: Spec
parseSExprSpec = describe "parseSExpr" $ do
  context "property based..." $ do
    it "parses single atoms successfully" $ require $ property $ do
      input <- forAll $ surroundWithZeroOrMore ' ' genAtom
      runParser parseSExpr input === Just ((A . atom) input, "")
    it "parses single level of atoms inside parens" $ require $ property $ do
    ...
    it "parses nested SExprs successfully" $ require $ property $ do
      ...
      let expected =
            Comb
              ([Comb (sexprs atoms1)] ++ sexprs atoms2 ++ [Comb (sexprs atoms3)]
              )
      runParser parseSExpr input === Just (expected, "")
  context "manual..." $ do

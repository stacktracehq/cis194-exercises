...
appendSpec =
  describe "+++" $
    it "concats two JoinLists" $
      joinListToList (exampleList +++ exampleList) `shouldBe` "yeahyeah"

indexJSpec :: Spec
indexJSpec =
  describe "indexJ" $ do
    it "returns Nothing for positive index out of bounds" $
      indexJ 100 exampleListSized `shouldBe` Nothing
    it "returns Nothing for negative index" $
      indexJ (-1) exampleListSized `shouldBe` Nothing
...
dropJSpec =
  describe "dropJ" $ do
    it "dropping from empty list gives empty list" $
      dropJ 1 empty `shouldBe` empty
    it "dropping more elements than are in list gives empty list" $
      dropJ 8 exampleListSized `shouldBe` empty
    it "joinListToList (dropJ n jl) == drop n (joinListToList jl)" $ do
      joinListToList (dropJ 0 exampleListSized) `shouldBe` joinListToList exampleListSized
      joinListToList (dropJ 1 exampleListSized) `shouldBe` drop 1 (joinListToList exampleListSized)
      joinListToList (dropJ 2 exampleListSized) `shouldBe` drop 2 (joinListToList exampleListSized)
      joinListToList (dropJ 3 exampleListSized) `shouldBe` drop 3 (joinListToList exampleListSized)

takeJSpec :: Spec
takeJSpec =
  describe "takeJ" $ do
    it "taking from empty list gives empty list" $
      takeJ 1 empty `shouldBe` empty
    it "taking more than the length of the list gives the whole list" $
      takeJ 8 exampleListSized `shouldBe` exampleListSized
    it "joinListToList (takeJ n jl) == take n (joinListToList jl)" $ do
      joinListToList (takeJ 0 exampleListSized) `shouldBe` []
      joinListToList (takeJ 1 exampleListSized) `shouldBe` take 1 (joinListToList exampleListSized)
      joinListToList (takeJ 2 exampleListSized) `shouldBe` take 2 (joinListToList exampleListSized)
      joinListToList (takeJ 3 exampleListSized) `shouldBe` take 3 (joinListToList exampleListSized)
...
scoreStringSpec =
  describe "scoreString" $ do
    ...
    it "scores string of special chars as 0" $
      scoreString "$#@%^!& *&^{}" `shouldBe` Score 0
    it "scores 'cat' as 5" $
      scoreString "cat" `shouldBe` Score 5
    it "scores 'dog' as 5" $
      scoreString "dog" `shouldBe` Score 5
    it "scores 'quickly' as 25" $
      scoreString "quickly" `shouldBe` Score 25
    it "scores 'pizzazz' as 45" $
      scoreString "pizzazz" `shouldBe` Score 45

scoreLineSpec :: Spec
scoreLineSpec =
  describe "scoreLine" $ do
    it "scoreLine '' == Single (Score 0) ''" $
      scoreLine "" `shouldBe` Single (Score 0) ""
    it "scoreLine 'cat' == Single (Score 5) 'cat'" $
      scoreLine "cat" `shouldBe` Single (Score 5) "cat"
    it "scoreLine 'quickly' == Single (Score 25) 'quickly'" $
      scoreLine "quickly" `shouldBe` Single (Score 25) "quickly"
    it "scoreLine 'yay ' +++ scoreLine 'haskell!' is correct join list" $ do
      let result = Append (Score 23) (Single (Score 9) "yay ") (Single (Score 14) "haskell!")
  ...
  describe "instance Buffer (JoinList (Score, Size) String)" $ do
    describe "toString" $ do
      it "empty join list is empty string" $
        toString emptyJoinList `shouldBe` ""
      it "join list with single element should be the single value" $
        toString l1 `shouldBe` "cat quickly"
      it "join list with 2 elements should be the string concatenation of the values" $
        toString l3 `shouldBe` "cat quicklyyay haskell!"
    describe "fromString" $ do
      it "empty string gives empty list" $
        fromString "" `shouldBe` emptyJoinList
      it "string without new line => correctly scored single element list" $
        fromString "cat quickly" `shouldBe` l1
      it "string with newline => correctly scored 2-elem list" $
        fromString "cat quickly\nyay haskell!" `shouldBe` l3
    describe "line" $ do
    ...
    describe "replaceLine" $ do
      it "replacing line in empty list returns empty list" $
        replaceLine 4 replacement emptyJoinList `shouldBe` emptyJoinList
      it "replaceing line with index out of bounds returns input list unchanged" $
        replaceLine 100 replacement l3 `shouldBe` l3
    ...
    describe "numLines" $ do
      it "returns 0 for empty list" $
        numLines emptyJoinList `shouldBe` 0
      it "returns 1 for singleon list" $
        numLines l1 `shouldBe` 1
      it "returns 2 for 2-elem list" $
        numLines l3 `shouldBe` 2
    describe "value" $
      it "retruns correct score value" $ do
